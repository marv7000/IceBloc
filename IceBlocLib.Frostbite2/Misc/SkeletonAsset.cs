...
public class SkeletonAsset
{
    public static InternalSkeleton ConvertToInternal(in Dbx dbx)
    {
        ...
        foreach (var boneName in (b.Value as Frostbite2.Complex).Fields)
        {
            boneNames.Add((string)boneName.Value);
        }

        b = dbx.Prim["Hierarchy"];
        foreach (var index in (b.Value as Frostbite2.Complex).Fields)
        {
            boneHierarchy.Add((int)index.Value);
        }

        for (int i = 0; i < (b.Value as Frostbite2.Complex).Fields.Count; i++)
        {
            Vector3 right = new(
