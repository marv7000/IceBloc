...
public partial class DctAnimation : Animation
{
    ...
    public float[] GenerateCoeffs(ushort p_Frame)
    {
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            var s_Multiplier = ((float)QuantizeMultSubblock *0.1f * (float)i + 1.0f)/ (float)QuantizeMultBlock;
    ...
    public Vector4 UnpackVec(List<short> p_Values, ushort p_Frame)
    {
        var s_Result = new Vector4(0.0f);

        var s_Coefs = GenerateCoeffs(p_Frame);

        for(var i=0; i < 8; i++)
        {
            var s_Vec = new Vector4((float)p_Values[i*4 + 0], (float)p_Values[i * 4 + 1], (float)p_Values[i * 4 + 2], (float)p_Values[i * 4 + 3]);
    ...
    public List<Vector4> Decompress()
    {
        ...
        var s_BitReader = new BitReader(new MemoryStream(Data), 64, Endianness.BigEndian);


        List<List<short>> s_Blocks = new();
        for (var s_BlockFrame = 0; s_BlockFrame < (NumKeys + 7) / 8; s_BlockFrame++)
        {
            for (var s_DofIdx = 0; s_DofIdx < s_DofTable.Length; s_DofIdx++)
            {
                ...
                {
                    var s_X = s_BitReader.ReadIntHigh(s_Component.SafeBitsX(CatchAllBitCount));
                    var s_Y = s_BitReader.ReadIntHigh(s_Component.SafeBitsY(CatchAllBitCount));
                    var s_Z = s_BitReader.ReadIntHigh(s_Component.SafeBitsZ(CatchAllBitCount));
                    var s_W = s_BitReader.ReadIntHigh(s_Component.SafeBitsW(CatchAllBitCount));
        ...
        for (var s_Frame = 0; s_Frame < NumKeys; s_Frame++)
        {
            ...
            for (var s_DofIdx = 0; s_DofIdx < s_DofTable.Length; s_DofIdx++)
            {
                ...
                dat.Add(UnpackVec(s_Block, (ushort)s_Frame));
            }
        }
